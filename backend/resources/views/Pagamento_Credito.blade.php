...
@section('content')
<!-- Conteúdo principal da página de pagamento via Cartão de Crédito -->
            ...
            <div class="payment-card" style="background:linear-gradient(120deg,#23243a 60%,#2e8fff 100%);border-radius:18px;padding:24px 20px;box-shadow:0 2px 12px #2e8fff22;">
                ...
                <form id="form-credito" action="/finalizar-compra" method="POST" style="display:flex;flex-direction:column;gap:14px;">
                    ...
                    <div style="margin:10px 0;">
                        <label style="color:#fff;font-size:0.9rem;margin-bottom:8px;display:block;">Parcelas:</label>
                        <select name="parcelas" id="parcelas-select" style="width:100%;padding:11px;border-radius:7px;border:1px solid #3a3a4a;background:#23243a;color:#fff;">
...
<script>
// Calcular o valor total para parcelas
let valorTotal = 0;
...
function calcularParcelas() {
    ...
    // Taxa de juros mensal (2.5% ao mês)
    const taxaJuros = 0.025;
    
    for (let i = 1; i <= 12; i++) {
        let valorParcela, textoJuros;
        ...
        if (i <= 6) {
            ...
            const montante = valorTotal * Math.pow(1 + taxaJuros, i);
        ...
        option.value = i;
        option.textContent = `${i}x de R$ ${valorParcela.toFixed(2).replace('.', ',')} ${textoJuros}`;
